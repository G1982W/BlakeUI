...
**Why client-side?** Timezone is detected with <span className="break-all">`Intl.DateTimeFormat().resolvedOptions().timeZone`</span> inside `useEffect` to avoid hydration mismatches when server and client timezones differ.
