...
/**
 * Docs preview only: @container breakpoints + container-width resize handling.
 * Production component: `navbar-9`.
 */
...
interface Navbar9DocsPreviewProps {
  className?: string;
}

const Navbar9DocsPreview = ({ className }: Navbar9DocsPreviewProps) => {
  ...
  useEffect(() => {
    ...
    const syncOpenFromWidth = () => {
      if (el.getBoundingClientRect().width > MOBILE_BREAKPOINT) {
        setOpen(false);
      }
    };

    syncOpenFromWidth();
    const ro = new ResizeObserver(syncOpenFromWidth);
    ro.observe(el);
  ...
  return (
    <div ref={containerRef} className="@container w-full min-w-0">
      <section
        ...
        <div className="container mx-auto h-16 px-2">
          <div className="flex h-full items-center gap-4 justify-between">
            ...
            >
              ...
              <span className="hidden text-background @2xl:inline-block">
                BlakeUI
              </span>
            </a>
            <NavigationMenu className="hidden @3xl:flex" viewport={false}>
      ...
      <MobileNavigationMenu open={open} onOpenChange={setOpen} />
    </div>
  );
};
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        >
          <X className="size-6 stroke-[1.75]" />
        </SheetClose>
