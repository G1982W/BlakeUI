  ...
  return (
    <div className="space-y-4 rounded-lg border border-border bg-background p-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Fixed-height table with sticky header and scroll.</p>
      </div>
          ...
          <Table>
            <TableHeader>
              {table.getHeaderGroups().map((headerGroup: HeaderGroup<InventoryRow>) => (
                <TableRow key={headerGroup.id} className="sticky top-0 z-10 bg-muted/80 backdrop-blur">
      ...
      <p className="text-xs text-muted-foreground sm:hidden">Swipe horizontally to see all columns.</p>
    </div>
  );
}
