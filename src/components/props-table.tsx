...
export function PropsTable({ component }: PropsTableProps) {
  ...
  return (
    // <div className="my-6 overflow-hidden rounded-lg border bg-white ">
    // </div>
    <table className="w-full bg-code-background text-left text-sm">
      ...
      <tbody className="divide-y">
        {props.map((prop) => (
          <tr key={prop.name}>
            ...
            <td
              className="px-4 py-2 font-mono text-xs text-secondary-foreground max-w-[200px]"
              title={prop.type}
            >
              {prop.type}
            </td>
